...
const ThreadDisplay = () => {
  ...
  return (
    <div className="flex flex-col h-full no-scroll">
      ...
      <Separator />
      {thread ? 
        <div className='flex flex-col flex-1 overflow-scroll no-scroll'>
          <div className='flex items-center p-4'>
            <div className="flex items-center p-4 text-sm w-2/3">
              <Avatar>
                <AvatarImage alt='avatar'  />
                <AvatarFallback>
                  {(thread as any)?.emails[0]?.from?.name?.split(" ")
            ...
            {(thread as any).emails[0].sentAt && (<div className='ml-auto text-xs text-muted-foreground'>
                {format(new Date((thread as any).emails[0].sentAt), 'PPpp')}
              </div>)}
          </div>
          <Separator />
          <div className='max-h-[calc(100vh-320px)] overflow-y-scroll flex flex-col no-scroll'>
