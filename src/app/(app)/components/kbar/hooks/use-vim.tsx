import { api, RouterOutputs } from '@/modules/shared/infrastructure/trpc/react';
...
const useVim = () => {
  ...
  const setUndone = api.thread.setUndone.useMutation({
    onMutate: async (payload: any) => {
      if (!payload.threadId && !payload.threadIds) return;
      if (!done) return;
      toast.success('Undone!');
      await queryClient.cancelQueries({ queryKey });
      const previousData = queryClient.getQueryData<RouterOutputs['thread']['listThreads']>(queryKey);
      queryClient.setQueryData(queryKey, (data: RouterOutputs['thread']['listThreads'] | undefined) => {
        if (!data) return data;
  ...
  const setDone = api.thread.setDone.useMutation({
    onMutate: async (payload: any) => {
      if (done) return;
      if (!payload.threadId && !payload.threadIds) return;
      toast.success('Done!');
